...
class NoiseEstimator {
 public:
  ...
  void PostUpdate(const float* speech_probability,
                  const float* signal_spectrum);

  // Returns the noise spectral estimate.
  const float* get_noise_spectrum() const {
    return noise_spectrum_.data();
  }

  // Returns the noise from the previous frame.
  const float* get_prev_noise_spectrum() const {
    return prev_noise_spectrum_.data();
  }

  // Returns a noise spectral estimate based on white and pink noise parameters.
  const float* get_parametric_noise_spectrum() const {
    return parametric_noise_spectrum_.data();
  }
  const float* get_conservative_noise_spectrum() const {
    return conservative_noise_spectrum_.data();
  }
