...
size_t NumBandsForRate(size_t sample_rate_hz) {
  if (sample_rate_hz == 16000) return 1;
  if (sample_rate_hz == 32000) return 2;
  if (sample_rate_hz == 48000) return 3;
  return 1;
}
...
void ApplyFilterBankWindow(std::array<float, kFftSize>& x) {
...
void FormExtendedFrame(const float* frame,
                       std::array<float, kFftSize - kNsFrameSize>& old_data,
                       std::array<float, kFftSize>& extended_frame) {
  std::copy(old_data.begin(), old_data.end(), extended_frame.begin());
  std::copy(frame, frame + kNsFrameSize, extended_frame.begin() + old_data.size());
...
void OverlapAndAdd(const std::array<float, kFftSize>& extended_frame,
                   std::array<float, kOverlapSize>& overlap_memory,
                   float* output_frame) {
  for (size_t i = 0; i < kOverlapSize; ++i) {
    output_frame[i] = overlap_memory[i] + extended_frame[i];
  }
  std::copy(extended_frame.begin() + kOverlapSize, extended_frame.begin() + kNsFrameSize, output_frame + kOverlapSize);
  std::copy(extended_frame.begin() + kNsFrameSize, extended_frame.end(), overlap_memory.begin());
}

// Produces a delayed frame.
void DelaySignal(const float* frame,
                 std::array<float, kFftSize - kNsFrameSize>& delay_buffer,
                 float* delayed_frame) {
  constexpr size_t kSamplesFromFrame = kNsFrameSize - (kFftSize - kNsFrameSize);
  std::copy(delay_buffer.begin(), delay_buffer.end(), delayed_frame);
  std::copy(frame, frame + kSamplesFromFrame, delayed_frame + delay_buffer.size());

  std::copy(frame + kSamplesFromFrame, frame + kNsFrameSize, delay_buffer.begin());
}

// Computes the energy of an extended frame.
float ComputeEnergyOfExtendedFrame(const std::array<float, kFftSize>& x) {
  float energy = 0.f;
  for (float x_k : x) {
    energy += x_k * x_k;
  }
...
float ComputeEnergyOfExtendedFrame(
  ...
  for (size_t i = 0; i < kNsFrameSize; ++i) {
...
void ComputeMagnitudeSpectrum(
    const std::array<float, kFftSize>& real,
    const std::array<float, kFftSize>& imag,
    std::array<float, kFftSizeBy2Plus1>& signal_spectrum) {
  signal_spectrum[0] = std::abs(real[0]) + 1.f;
  signal_spectrum[kFftSizeBy2Plus1 - 1] = std::abs(real[kFftSizeBy2Plus1 - 1]) + 1.f;

  for (size_t i = 1; i < kFftSizeBy2Plus1 - 1; ++i) {
    signal_spectrum[i] = std::sqrt(real[i] * real[i] + imag[i] * imag[i]) + 1.f;
  }
}

// Compute prior and post SNR.
void ComputeSnr(const float* filter,
                const float* prev_signal_spectrum,
...
float ComputeUpperBandsGain(
    ...
    const float* filter,
    const float* speech_probability,
    const float* prev_analysis_signal_spectrum,
    const float* signal_spectrum) {
...
NoiseSuppressor::NoiseSuppressor(const NsConfig& config,
  ...
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    channels_[ch] = std::unique_ptr<ChannelState>(new ChannelState(suppression_params_, num_bands_));
  }
}

void NoiseSuppressor::AggregateWienerFilters(float* filter) const {
  const float* filter0 = channels_[0]->wiener_filter.get_filter();
  std::copy(filter0, filter0 + kFftSizeBy2Plus1, filter);

  for (size_t ch = 1; ch < num_channels_; ++ch) {
    const float* filter_ch = channels_[ch]->wiener_filter.get_filter();
...
void NoiseSuppressor::Analyze(const float* audio, size_t length) {
  ...
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    const float* y_band0 = audio + ch * length;
  ...
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    ...
    // presence.
    ch_p->noise_estimator.PreUpdate(num_analyzed_frames_, signal_spectrum.data(), signal_spectral_sum);
    ...
    ComputeSnr(ch_p->wiener_filter.get_filter(),
               ch_p->prev_analysis_signal_spectrum.data(),
               signal_spectrum.data(),
    ...
    ch_p->noise_estimator.PostUpdate(
        ch_p->speech_probability_estimator.get_probability(),
        signal_spectrum.data());
...
void NoiseSuppressor::Process(float* audio, size_t length) {
  ...
  FilterBankState* filter_bank_states = filter_bank_states_stack.data();
  std::array<float, kMaxNumChannelsOnStack> upper_band_gains_stack;
  float* upper_band_gains = upper_band_gains_stack.data();
  std::array<float, kMaxNumChannelsOnStack> energies_before_filtering_stack;
  float* energies_before_filtering = energies_before_filtering_stack.data();
  std::array<float, kMaxNumChannelsOnStack> gain_adjustments_stack;
  float* gain_adjustments = gain_adjustments_stack.data();
  if (NumChannelsOnHeap(num_channels_) > 0) {
    // If the stack-allocated space is too small, use the heap for storing the
    // data.
    filter_bank_states = filter_bank_states_heap_.data();
    upper_band_gains = upper_band_gains_heap_.data();
    energies_before_filtering = energies_before_filtering_heap_.data();
    gain_adjustments = gain_adjustments_heap_.data();
  }

  // Compute the suppression filters for all channels.
  for (size_t ch = 0; ch < num_channels_; ++ch) {
    // Form an extended frame and apply analysis filter bank windowing.
    float* y_band0 = audio + ch * length;
    ...
    if (num_bands_ > 1) {
      ...
      upper_band_gains[ch] = ComputeUpperBandsGain(
          ...
          channels_[ch]->prev_analysis_signal_spectrum.data(),
          signal_spectrum.data());
    }
  }
  ...
  const float* filter = filter_data.data();
  if (num_channels_ == 1) {
    filter = channels_[0]->wiener_filter.get_filter();
  } else {
    AggregateWienerFilters(filter_data.data());
  }
  ...
  if (num_bands_ > 1) {
    ...
    for (size_t ch = 0; ch < num_channels_; ++ch) {
      for (size_t b = 1; b < num_bands_; ++b) {
        ...
        // the lowest band.
        float* y_band = audio + ch * length; // Simplified for single band
        std::array<float, kNsFrameSize> delayed_frame;
        DelaySignal(y_band, channels_[ch]->process_delay_memory[b - 1], delayed_frame.data());
